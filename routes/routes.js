...
async function getFriendsId(steam64) {
  let url =
    `/ISteamUser/GetFriendList/v1/?relationship=friend&steamid=` + steam64;
  let friendsId = [];
  try {
    await steam
      .get(url, this.baseAPI, process.env.STEAM_API_KEY)
      .then((response) => {
        friendsId = response;
      });
    return friendsId;
...
router.post("/api/steam/games", async (req, res) => {
  ...
  try {
    ...
    if (regExp.test(user_id_req)) {
      ...
      let id = await steam.resolve(
        "https://steamcommunity.com/id/" + user_id_req
      );
      user_url =
        "/IPlayerService/GetOwnedGames/v1?include_played_free_games=true&include_appinfo=true&format=json&steamid=" +
        id;
      user_id = id;
  ...
  try {
    ...
    await steam
      .get(user_url, this.baseAPI, process.env.STEAM_API_KEY)
      .then((data) => {
        user_games = data;
      });
