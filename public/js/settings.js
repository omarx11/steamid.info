...
async function updateGame(selected) {
  ...
  const save = await saveSelectedPrimaryGame(
    selectedGameId,
    selectedGameName,
    selectedGameImg
  );
...
function startAddingGame1() {
  game_list_btn.addEventListener("click", () =>
    game_list.classList.toggle("active")
  );
  add_game();
  ...
  games_search.addEventListener("keyup", () => {
    ...
    arr = infos.gameData
      ...
      .map((data) => {
        let isSelected =
          data == game_list_btn.firstElementChild.innerText ? "selected" : "";
        return `<li value="${data.appid}" onclick="updateGame(this)" class="${isSelected}">${data.name}</li>`;
      })
      .join("");
    games_options.innerHTML = arr
      ? arr
      : `<p style="margin-top: 10px; color: #ccc; text-align: left">Oops! game not found</p>`;
  });
}
...
async function updateGame2(selected) {
  ...
  let selectedGameName = selected.textContent;
  let selectedGameImg = `https://cdn.cloudflare.steamstatic.com/steam/apps/${selectedGameId}/library_600x900.jpg`;
  const save = await saveSelectedSecondaryGame(
    selectedGameId,
    selectedGameName,
    selectedGameImg
  );
...
function startAddingGame2() {
  game_list_btn2.addEventListener("click", () =>
    game_list2.classList.toggle("active")
  );
  add_game2();
  ...
  games_search2.addEventListener("keyup", () => {
    ...
    arr = infos.gameData
      ...
      .map((data) => {
        let isSelected =
          data == game_list_btn2.firstElementChild.innerText ? "selected" : "";
        return `<li value="${data.appid}" onclick="updateGame2(this)" class="${isSelected}">${data.name}</li>`;
      })
      .join("");
    games_options2.innerHTML = arr
      ? arr
      : `<p style="margin-top: 10px; color: #ccc; text-align: left">Oops! game not found</p>`;
  });
}
